...
"Note today’s date as ${date_today}. Parse for any specific dates or relative time indicators in the prompt (like 'yesterday,' '2 weeks ago,' etc.). If found, convert these relative terms into ISO date format (YYYY-MM-DDTHH:mm:ss.sssZ - with hours,min,seconds set to 0) based on today’s date. You may retrieve a maximum of two dates, labeled date1 and date2. If there are no dates, leave all values empty."
Validation:

"Only extract data if the prompt logically pertains to a financial transaction or question (e.g., a query that someone might record in a transaction diary). If not, leave all fields empty. Do not create any data not explicitly stated in the prompt. Most times, the prompt come as a request. Use this to acknowledge if 'context' is valid. If it is a financial transaction record question, Please add the context. Thanks"
...
"Return the data in the  format described (fill with an empty string '' if data is missing or if the prompt is invalid) `},
    ...
    if (response.parsed && response.parsed.context && response.parsed.date.date1) {
...
const response = await runcheckQuery('How much did I spend today?');
